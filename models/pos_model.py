...
def get_pos_referentiel_count(
    ...
    Utilise les colonnes natives de referentiel_pos (zone_centre, zone_territoire,
    secteur_cluster, zone_sa, segment_group) en appliquant les valeurs issues
    des options de filtres de la vue OOS (zone, territory, cluster).
    Les filtres zone/territory sont cherchés sur zone_centre ET zone_territoire
    pour tolérer les variantes de nommage entre listing_oos et le référentiel.
    """
    close = False
    if conn is None:
        conn = get_connection()
    ...
    try:
        ...
        if zone and zone not in ("Tous", "Toutes", ""):
            # Cherche dans zone_centre (colonne native du référentiel)
            where_clauses.append(
                "(zone_centre = ? OR LOWER(zone_centre) = LOWER(?))"
            )
        ...
        if cluster and cluster not in ("Tous", "Toutes", ""):
            # secteur_cluster dans le référentiel = cluster dans listing_oos
            where_clauses.append(
                "(secteur_cluster = ? OR LOWER(secteur_cluster) = LOWER(?))"
            )
            params.extend([cluster, cluster])

        if zone_sa and zone_sa not in ("Tous", "Toutes", ""):
            where_clauses.append(
                "(zone_sa = ? OR LOWER(zone_sa) = LOWER(?))"
            )
            params.extend([zone_sa, zone_sa])

        if segment_group and segment_group not in ("Tous", "Toutes", ""):
            where_clauses.append(
                "(segment_group = ? OR LOWER(segment_group) = LOWER(?))"
            )
